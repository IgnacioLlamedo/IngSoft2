	...
	try {
		...
		if (!user) {
			return res.json({
				success: false,
				message:
					"Error al Iniciar Sesión en la cuenta. El email ingresado no está registrado.",
			});
		}
		...
		if (user.contraseña !== password) {
			return res.json({
				success: false,
				message:
					"Error al Iniciar Sesión en la cuenta. La contraseña ingresada es incorrecta.",
			});
		}
...
export async function crearCodigo(req, res) {
	try {
		...
		res.json({
			success: false,
			message:
				"Error al crear código de autenticación. Inténtelo más tarde.",
		});
	}
}
