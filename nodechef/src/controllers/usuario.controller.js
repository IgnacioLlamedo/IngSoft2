...
export async function almacenarPagoController(req, res){
    try {
        ...
        //await pagoDao.create(dataPago);   Crea el nuevo pago y lo almacena en DB

        const redirect = '/my-activities'; //Redirigir a las actividades del usuario una vez que el pago se guarde en DB
        res.json({
            success: true,
            redirect
        })
